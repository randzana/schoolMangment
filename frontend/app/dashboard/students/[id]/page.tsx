  ...
  if (error || !student) {
    return (
      <div className="p-6 text-center text-danger font-medium border border-danger/20 rounded-xl bg-danger/5">
        نەتوانرا زانیاری قوتابی باربکرێت. پێدەچێت ئەم تۆمارە بوونی نەبێت.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <span>گەڕانەوە بۆ قوتابییەکان</span>
      </button>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${student.is_active ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'}`}>
                {student.is_active ? 'چالاک' : 'ناچالاک'}
              </span>
            </div>
            <p className="text-xs text-text-muted mt-0.5">
              ژمارەی سێریال: <span className="font-semibold text-text font-mono">{student.serial_number}</span> · پۆل / قۆناغ: <span className="font-semibold text-text">{gradeDisplay(student.grade)}</span>
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-3 space-y-6">
          ...
          <div className="bg-white border border-border rounded-xl shadow-card p-6">
            <h3 className="font-semibold text-sm text-text border-b pb-2 mb-4">پارەی خوێندن (ساڵانە)</h3>
            {student.study_payments && student.study_payments.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-border text-xs text-left">
                  <thead>
                    <tr className="text-text-muted font-semibold bg-surface-muted">
                      <th className="px-4 py-2">ساڵی خوێندن</th>
                      <th className="px-4 py-2">کرێی ساڵانە</th>
              ...
              <p className="text-xs text-text-muted text-center py-6">هیچ تۆمارێکی پارەی خوێندن نییە.</p>
            )}
          </div>

          {/* Food Payment Summary */}
          <div className="bg-white border border-border rounded-xl shadow-card p-6">
            <h3 className="font-semibold text-sm text-text border-b pb-2 mb-4">پارەی نانخواردن (مانگانە)</h3>
            {student.food_payments && student.food_payments.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-border text-xs text-left">
                  <thead>
                    <tr className="text-text-muted font-semibold bg-surface-muted">
                      <th className="px-4 py-2">ساڵی خوێندن</th>
                      <th className="px-4 py-2">کرێی مانگانە</th>
                      <th className="px-4 py-2">داشکاندن</th>
                      <th className="px-4 py-2">کۆیی گشتی</th>
                      <th className="px-4 py-2">کۆی دراو</th>
                      <th className="px-4 py-2">قەرز (ماوە)</th>
              ...
              <p className="text-xs text-text-muted text-center py-6">هیچ تۆمارێکی پارەی نانخواردن نییە.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="bg-white border border-border rounded-xl shadow-card p-6">
          <h3 className="font-semibold text-sm text-text border-b pb-2 mb-4">مامەڵەکانی قستی خوێندن</h3>
          {student.study_installments && student.study_installments.length > 0 ? (
            ...
            <p className="text-xs text-text-muted text-center py-6">هیچ مامەڵەیەکی خوێندن تۆمار نەکراوە.</p>
          )}
        </div>
      ...
      {/* Edit Profile Modal */}
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="دەستکاریکردنی زانیاری قوتابی">
