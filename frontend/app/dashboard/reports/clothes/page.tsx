...
export default function ClothesReportPage() {
  const [month, setMonth] = useState('');
  const [grade, setGrade] = useState('');
 
  const { data: reportResponse, isLoading } = useQuery({
    queryKey: ['report-clothes-payments', month, grade],
    queryFn: async () => {
      const res = await api.get('/reports/clothes-payments', {
        params: { month, grade },
      });
  ...
  const handleExportCsv = () => {
    ...
    link.href = url;
    link.download = `clothes_report_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
 
  const handlePrintPdf = () => {
    window.open(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/reports/clothes-payments/pdf?month=${month}&grade=${grade}`,
      '_blank'
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="bg-primary/5 p-4 border border-primary/20 rounded-xl">
          <span className="text-[10px] text-primary font-bold">کۆی داهات (جلوبەرگ)</span>
