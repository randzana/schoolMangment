...
export default function FoodIncomeReportPage() {
  const [year, setYear] = useState(String(new Date().getFullYear()));
 
  const { data: reportResponse, isLoading } = useQuery({
    queryKey: ['report-food-income', year],
    queryFn: async () => {
      const res = await api.get('/reports/food-income', {
        params: { year },
      });
  ...
  const handleExportCsv = () => {
    window.open(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/reports/food-income/export?year=${year}`,
      '_blank'
    );
  };
 
  const columns: Column<any>[] = [
    { header: 'مانگ', accessor: (row) => {
        // Translate months to Kurdish
        ...
        return monthMap[row.month] || row.month;
      }
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-primary/5 p-5 border border-primary/20 rounded-xl max-w-sm">
        <span className="text-[10px] text-primary font-bold">کۆی گشتی داهاتی ساڵانەی نانخواردن</span>
      ...
      <div className="bg-white border rounded-xl shadow-card p-6">
        <h3 className="font-semibold text-xs text-text mb-4 tracking-wider">داهاتی مانگانەی نانخواردن (بە دینار)</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={months.map((m: any) => {
              // Convert English month name to Kurdish for chart display
              const monthMap: Record<string, string> = {
                January: '١',
                February: '٢',
                March: '٣',
              ...
              return { ...m, displayMonth: monthMap[m.month] || m.month };
            })} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="displayMonth" stroke="#A0AEC0" fontSize={10} tickLine={false} />
