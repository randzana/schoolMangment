...
import { useFoodPayments, useFoodPaymentsSummary, useSaveFoodPayment } from '@/hooks';
...
type SetupFormValues = zod.infer<typeof setupFoodSchema>;
...
export default function FoodPaymentsPage() {
  ...
  const { data: paymentsData, isLoading } = useFoodPayments({ search, grade, page });
  const { data: summaryData } = useFoodPaymentsSummary();
  const savePaymentMutation = useSaveFoodPayment();
  ...
  const {
    ...
    resolver: zodResolver(setupFoodSchema),
  });

  const monthlyPrice = watch('monthly_price') || 0;
  ...
  const openAddModal = () => {
    ...
    reset({
      student_id: 0,
      monthly_price: 150000, // default rate
      discount: 0,
      notes: '',
    });
    setIsFormOpen(true);
  ...
  const openEditModal = (payment: any) => {
    ...
    reset({
      ...
      monthly_price: parseFloat(payment.monthly_price) || 0,
