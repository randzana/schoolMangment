...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'accent' | 'danger' | 'success' | 'warning' | 'ghost';
  size?: 'sm' | 'md' | 'lg';
  isLoading?: boolean;
}
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className = '', variant = 'primary', size = 'md', isLoading, children, disabled, ...props }, ref) => {
    ...
    const variants = {
      ...
      secondary: 'bg-surface-muted hover:bg-gray-200 text-text border border-border focus:ring-gray-400',
      accent: 'bg-accent hover:bg-amber-500 text-white focus:ring-accent',
      danger: 'bg-danger hover:bg-red-700 text-white focus:ring-danger',
      success: 'bg-success hover:bg-emerald-700 text-white focus:ring-success',
      warning: 'bg-warning hover:bg-orange-600 text-white focus:ring-warning',
      ghost: 'bg-transparent hover:bg-gray-100 text-text-muted hover:text-text focus:ring-gray-300',
    };
